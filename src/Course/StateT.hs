...
instance Bind f => Apply (StateT s f) where
  (<*>) ::
    ...
    -> StateT s f b
  (<*>) stf st = StateT(\s ->
                    (runStateT stf s) >>= \(fn, s') -> ( (\(a,s'') -> (fn a, s'')) <$> (runStateT st s') )
                    )
...
instance Monad f => Bind (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f b
  (=<<) fnast st = StateT(\s ->
                    (runStateT st s) >>= \(a, s') -> runStateT (fnast a) s'
                    )
...
execT ::
  ...
  -> s
  -> f s
execT st s = (\(_, s') -> s') <$> (runStateT st s)
...
evalT ::
  ...
  -> s
  -> f a
evalT st s = (\(a,_) -> a) <$> (runStateT st s)
...
distinct' l = eval (filtering p l) S.empty
            where p = (\e -> State(\set -> (not $ S.member e set, S.insert e set)))
--distinct' = error "todo"
...
distinctF l = evalT (filtering p l) S.empty
            where p = (\e -> StateT(\set -> if (e>100) then Empty else Full(not $ S.member e set, S.insert e set)))
            --where p = (\e -> if (e>100) then StateT(\_ -> Empty) else StateT(\set -> Full(not $ S.member e set, S.insert e set)))
...
instance Functor f => Functor (OptionalT f) where
  ...
  (<$>) fnab opta = OptionalT(
                    let mopa = (runOptionalT opta)
                    in (\opa -> mapOptional fnab opa) <$> mopa
                    )
...
instance Apply f => Apply (OptionalT f) where
  ...
  (<*>) optfnab opta = OptionalT(
                       let mopfnab = runOptionalT optfnab
                           mopa = runOptionalT opta
                       in lift2 applyOptional mopfnab mopa
                       )

-- | Implement the `Applicative` instance for `OptionalT f` given a Applicative f.
instance Applicative f => Applicative (OptionalT f) where
  pure a = OptionalT(pure (Full a))
  -- or --pure = OptionalT . pure . Full
...
instance Monad f => Bind (OptionalT f) where
  ...
  (=<<) fnaoptb opta = OptionalT((runOptionalT opta) >>= \opa -> case opa of
                                                                      Empty -> return Empty
                                                                      Full a -> runOptionalT(fnaoptb a))
...
instance Bind (Logger l) where
  ...
  (=<<) fn (Logger l1 a) = Logger (l1++l2) b
                         where Logger l2 b = fn a
...
distinctG l = runOptionalT ( evalT (filtering p l) (S.empty) )
            where p = (\e ->
                      StateT(\set -> if (e>100) then OptionalT ((log1 ("aborting > 100: "++listh(show e))) Empty)
                                     else OptionalT (
                                        (
                                          if (e `mod` 2 == 0) then (log1 ("even number: "++listh(show e)))
                                          else (Logger Nil)
                                        )
                                        (Full(not (S.member e set), S.insert e set))
                                     )
                      ))
