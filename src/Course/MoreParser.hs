...
--stringTok s = string s >>= \v -> spaces >>> return v
...
between ::
  ...
  -> Parser a
  -> Parser a
between p1 p2 p3 = p1 >>= \_ -> p3 >>= \v -> p2 >>= \_ -> return v
...
hex ::
  Parser Char
hex = thisMany 4 (satisfy (\c -> isHexDigit c)) >>= \hx -> case readHex hx of
                                                            Empty -> return $ chr 0
                                                            Full n -> return $ chr n
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 pa ps = pa >>= (\c -> spaces >>> list (ps >>> pa) >>= \r -> return (c:.r))
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll lp = character >>= \c -> if and $ (sequence lp) c
                                    then return c
                                    else failed
{- or
satisfyAll lp = character >>= \c -> if (all (\p -> p c) lp)
                                    then return c
                                    else failed
-}
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny lp = character >>= \c -> if or $ (sequence lp) c
                                    then return c
                                    else failed
